		...
		if not marked[tid]:
			...
			for jj in range(rowlength[tid]):
				...
				if marked[nid]:
					...
					eps = 1e-6
					tval = vv[rs+jj] * (1.0/(weights[tid]+eps) + 1.0/(weights[nid]+eps))
				if tval > wmax:
					wmax = tval
					bestneighbor = nid
...
# Hryhorii Chereda has added this 2 functions below
def perm_data_back(x, indices, feature_num):
    """
    ...
    # print(order)
    x_to_origin = x[:, order]
    return x_to_origin[:,0:feature_num]


def perm_matrix_back(mat, indices, feature_num):
    """
    ...
    if indices is None:
        return mat
    order = np.argsort(indices)
    mat_origin = mat[:, order]
    mat_origin = mat_origin[order, :]
    return mat_origin[0:feature_num, 0:feature_num]
